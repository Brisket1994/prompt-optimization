...
Live calibration (Phase 1.5) is mandatory on every run — it catches model, API, and best-practice changes since the skill was last edited, so guidance never goes stale. It also re-verifies the Fable 5 / Mythos 5 system-card behavioral findings the Phase 3 anti-pattern sweep relies on: the six-pattern long-horizon failure taxonomy, the Mythos-vs-Fable architecture and fallback semantics, elevated eval/grader-awareness, the over-refusal inversion (~35× drop on API), the mixed prompt-injection picture (improved on most surfaces, regressed on browser-legacy and prefill), and the new long-run pathologies (context anxiety, simulation-framing rationalization, survival-pressure framing, multi-agent turf wars).
...
- **2-O (orchestrated fan-out)** — when the draft is multi-sub-domain, multi-stakeholder, or contested, the skill fans out via a Claude Code **dynamic workflow** following one of the three card-documented harness patterns (blocking orchestrator + subagents; fixed-agent peer team; async lead-spawns-long-lived-subagents) with a **preference for non-blocking patterns** (Meta-Rule 17 — non-blocking dominates blocking on latency and tokens; advantage concentrates on the hard long-tail). One `landscape-research` workflow agent per lane plus a dedicated adversarial lane, each searching deeply in its own isolated context, then synthesizes the aggregate. Bounded by ~6 lanes (synthesis bandwidth), per-lane budgets, and one remediation pass. Falls back to parallel subagents via the Agent/Task tool when the workflow runtime is unavailable.

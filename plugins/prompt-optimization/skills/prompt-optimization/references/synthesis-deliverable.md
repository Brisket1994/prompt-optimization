...
> **Calibration stamp:** This reference is skill-internal methodology (deliverable structure, evidentiary-chain discipline, adversarial-pass protocol), not Anthropic product facts — it does not require Phase 1.5 re-verification. Reference last edited 2026-06-10; methodology unchanged thereafter unless this stamp is bumped.
...
- **The overall conclusion is checked against the mission, not an intermediate milestone.** The Fable 5 / Mythos 5 system card §4 documents the model treating an intermediate milestone (a lane returning "done") as the finish line; the cluster "reported work as done or verified when it wasn't" is 16/886 transcripts (card 1839–1865). Before finalizing, the reduce step compares the overall conclusion to the orchestrator's `## Mission` statement and confirms the conclusion answers the mission — not a subordinate question that surfaced along the way. (`fable-5-system-card.md` §4.)
- **Lane uncertainty is preserved, not dropped.** The Fable 5 system card §4 documents dropping a subagent's caveat and relaying a "could not verify" guess as a confirmed fact (the "states an unverified guess as fact" cluster is 41/886 transcripts; card 1733–1798). The reduce step carries each lane's caveats verbatim and treats "the lane could not verify X" as a negative result that lowers the key finding's confidence (or strikes it), never as a fact to relay confidently. This is the upstream complement to the source-validation revisit pass. (`fable-5-system-card.md` §4.)

...
> **Calibration stamp:** Methodology verified against the skill's own protocol design on **2026-05-20**. The query taxonomy and source-type diversity requirements are skill-internal methodology, not Anthropic product facts — they do not require Phase 1.5 re-verification, but the worked search examples should use the session's real current date.
...
| Condition | Mode |
|---|---|
| Agent/Task tool unavailable (claude.ai chat, API without a dispatch harness) | **2-S** (single-threaded) — mandatory fallback |
| Agent/Task tool available AND draft is single-sub-domain / well-bounded | **2-S** — orchestration overhead not warranted |
| Agent/Task tool available AND draft is multi-sub-domain, multi-stakeholder, or contested (the same depth signals that select the Full track at Phase 0.5) | **2-O** (orchestrated fan-out) |

Check Agent/Task availability the same way the Widget Interaction Protocol checks for `ask_user_input_v0` — inspect the current tool surface before committing to a mode. Selection is by these objective criteria, not planner discretion; report the chosen mode and a one-line rationale in the chat run sheet and the Phase 5 delta. A user override to 2-S is always honored; an override to 2-O is honored only when the Agent/Task tool is actually available.
...
2-O reuses the subagent machinery documented in [orchestrated-research.md](orchestrated-research.md) rather than re-deriving it. Read that file's §2 (decomposition), §3 (subagent prompt construction), §5 (shared constants), §6 (output scope calibration), §7 (synthesis), §8 (remediation), and §9 (environment configuration) — those rules apply verbatim.

**Dispatch vehicle.** When the prompt-optimization plugin is installed, dispatch each lane as the bundled **`landscape-research`** agent — it already encodes the read-only research role, the neutrality discipline, and the synthesis-ready output shape, so the orchestrator supplies only the per-lane scope, the 2A shared-constant artifact, the per-lane query/source floors, and the search budget. When that named agent is unavailable (the skill running outside the plugin), construct the same brief inline from the [orchestrated-research.md](orchestrated-research.md) §3 mandatory elements plus the bindings below.

The Phase-2-specific bindings:

**1. The orchestrator runs 2A first, alone.** The 2A deconstruction artifact (topical content, implicit framing, stakeholder map, contested terminology) becomes the *shared constant* every subagent receives in its brief — the [orchestrated-research.md](orchestrated-research.md) §5 shared-constants pattern. This ensures each subagent targets the full landscape 2A surfaced, not just the narrow slice its lane covers. Never let a subagent re-derive 2A; cloning that step propagates the same blindspots across every lane ([orchestrated-research.md](orchestrated-research.md) §3, "boilerplate-cloned subagent prompts").

**2. Decompose along a Phase-2-native axis.** Pick the decomposition axis from [orchestrated-research.md](orchestrated-research.md) §2 that fits the draft. For landscape research the natural axes are **sub-domain** (one subagent per major sub-domain from 2A), **source-type** (one per Step 2B.2 source category), or **query-taxonomy cluster** (group the Step 2B.1 rows into lanes). Cap at ≤6 subagents — the synthesis-bandwidth limit from [orchestrated-research.md](orchestrated-research.md) §2.

**3. Always dedicate one subagent to the adversarial pass (2B').** In 2-S, 2B' shares the orchestrator's 12-search budget; in 2-O it gets its own lane and its own budget, so the strongest counter-framing is searched at genuine parity. Its brief: steel-man the strongest argument against the draft's implicit thesis (drawn from 2A's implicit-framing and excluded-vantages findings) and search it at the same depth as the topic lanes — identifying what the dissenting framing is and where it is taken seriously, never adopting it as a conclusion.

**4. Each subagent brief carries — in addition to the [orchestrated-research.md](orchestrated-research.md) §3 mandatory elements:**
   ...
   - A per-lane search budget (3–6 searches typical) and a per-lane rabbit-hole guard: a subagent that cannot close its lane returns what it has and lists what remains, per [orchestrated-research.md](orchestrated-research.md) §6 "refusal-shaped output."
   ...
   - A synthesis-ready output shape: key findings first, then evidence, then unclosed gaps, then sources — mirroring the 2C artifact fields so the orchestrator can assemble lanes without reformatting.

**5. Depth without recursion.** Aggregate search count in 2-O intentionally exceeds the 12-search single-thread cap — that expansion is the entire reason to orchestrate. It stays bounded by three guards rather than a global ceiling: ≤6 subagents, a per-lane search budget, and **at most one remediation pass** ([orchestrated-research.md](orchestrated-research.md) §8). If synthesis surfaces a material sub-domain no lane covered, dispatch exactly one scoped remediation subagent; a second remediation signals the 2A decomposition was wrong — return to 2A rather than chaining passes. Subagents cannot spawn subagents, so there is no deeper nesting to guard against.

**6. Synthesis is the orchestrator's job — never a subagent.** The orchestrator collects every subagent's findings pack and assembles the single 2C Landscape Synthesis artifact (below), then runs 2D and 2D' on the **aggregate corpus** — all lanes combined — not per lane. Source-type diversity (Step 2B.2 / the 2D' source axis) is verified across the full cross-lane corpus, exactly as [orchestrated-research.md](orchestrated-research.md) §7 task 3 specifies. The five mandatory synthesis tasks from [orchestrated-research.md](orchestrated-research.md) §7 (cross-subagent consistency, coverage, source diversity, bias, synthesized findings) are the 2-O mechanism for producing the 2C artifact and passing 2D/2D'.

**7. The partition holds identically.** Subagent briefs, the dispatch graph, and the per-lane findings packs are Phase 2 working machinery — they feed 2C and the chat run sheet, never the optimized prompt body or the written file. The Phase 6B pre-write strip check enforces this regardless of mode.

The subagent-cannot-spawn-subagents constraint and the under-delegation behavior are documented in [orchestrated-research.md](orchestrated-research.md) §1, the environment configuration in §9, and both trace to the sources listed at the end of that file (accessed 2026-05-20). This section adds only the Phase-2-specific bindings, which are skill-internal methodology.

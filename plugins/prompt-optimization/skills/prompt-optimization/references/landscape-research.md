...
2-O reuses the subagent machinery documented in [[orchestrated-research]] rather than re-deriving it. Read that file's §2 (decomposition), §3 (subagent prompt construction), §5 (shared constants), §6 (output scope calibration), §7 (synthesis), §8 (remediation), and §9 (environment configuration) — those rules apply verbatim.

**Dispatch vehicle.** When the prompt-optimization plugin is installed, dispatch each lane as the bundled **`landscape-research`** agent — it already encodes the read-only research role, the neutrality discipline, and the synthesis-ready output shape, so the orchestrator supplies only the per-lane scope, the 2A shared-constant artifact, the per-lane query/source floors, and the search budget. When that named agent is unavailable (the skill running outside the plugin), construct the same brief inline from the [[orchestrated-research]] §3 mandatory elements plus the bindings below.

The Phase-2-specific bindings:

**1. The orchestrator runs 2A first, alone.** The 2A deconstruction artifact (topical content, implicit framing, stakeholder map, contested terminology) becomes the *shared constant* every subagent receives in its brief — the [[orchestrated-research]] §5 shared-constants pattern. This ensures each subagent targets the full landscape 2A surfaced, not just the narrow slice its lane covers. Never let a subagent re-derive 2A; cloning that step propagates the same blindspots across every lane ([[orchestrated-research]] §3, "boilerplate-cloned subagent prompts").

**2. Decompose along a Phase-2-native axis.** Pick the decomposition axis from [[orchestrated-research]] §2 that fits the draft. For landscape research the natural axes are **sub-domain** (one subagent per major sub-domain from 2A), **source-type** (one per Step 2B.2 source category), or **query-taxonomy cluster** (group the Step 2B.1 rows into lanes). Cap at ≤6 subagents — the synthesis-bandwidth limit from [[orchestrated-research]] §2.
...
**4. Each subagent brief carries — in addition to the [[orchestrated-research]] §3 mandatory elements:**
   ...
   - A per-lane search budget (3–6 searches typical) and a per-lane rabbit-hole guard: a subagent that cannot close its lane returns what it has and lists what remains, per [[orchestrated-research]] §6 "refusal-shaped output."
...
The subagent-cannot-spawn-subagents constraint and the under-delegation behavior are documented in [[orchestrated-research]] §1, the environment configuration in §9, and both trace to the sources listed at the end of that file (accessed 2026-05-20). This section adds only the Phase-2-specific bindings, which are skill-internal methodology.
...
- See [[task-heuristics]] §Research / search for the query-taxonomy guidance that appears in the optimized prompt itself.
- See [[prompt-template]] for the embedding-landscape-without-bias table that governs how 2C findings land in the prompt body.

...
This reference is loaded by the Fable 5 Configuration section of `SKILL.md` and consulted whenever the optimized prompt's `<deployment_config>` block is being constructed. It captures the model's API surface, what carries forward from Opus 4.8, what is new in Fable 5 / Mythos 5, and the behaviors that should shape a Fable-5-targeted prompt.
...
The table partitions behaviors by whether they carry forward from Opus 4.8 or are new in Fable 5 / Mythos 5. Both halves matter — the 4.8-carryforward items are easy to forget because they already shaped 4.8 prompts; the Fable-5-new items are easy to miss because they invert prior assumptions (thinking-only model; over-refusal collapsed; injection improved on most surfaces but regressed on browser-legacy and prefill; honesty changed shape rather than improving uniformly).

### Carries forward from Opus 4.8

| Behavior | What this means for the optimized prompt |
|----------|-------------------------------------------|
| **Prefilling assistant messages returns 400.** Partial-turn prefill is not generally available externally for Opus 4.6+, Mythos 5, or Fable 5 (card 4530–4535). | Never instruct prefilling. For format control use structured outputs, system-prompt instructions, or `output_config.format`. |
...
| **Effort ladder is `low / medium / high / xhigh / max`.** | Effort levels carry the same names; Fable 5's saturation behavior by task class is what changed (see Effort levels below). |
...
| **Thinking-only model.** `thinking={"type":"adaptive"}` remains valid syntax; the old "adaptive thinking OFF by default on raw API" claim is **INVERTED** — there is no thinking-off mode on Fable / Mythos 5 (card 3082–3088). | A thinking-dependent prompt may behave differently when fallback to Opus 4.8 fires (Opus 4.8 still supports a thinking-off mode). Surface this as a deployment note for sensitive-domain runs. |
...
| **New safeguard categories: cyber, bio, chem, distillation, frontier-LLM-development.** Frontier-LLM-dev throttles **invisibly** via prompt modification / steering / PEFT and does **NOT** fall back, impacting ~0.03% of traffic (755–778). Binary reverse-engineering is **blocked on Fable 5** — ProgramBench excluded (9643–9667). Bio classifiers flag biology **images** too (LAB-Bench FigQA, 10463–10465). | For cyber/bio/sensitive-domain deliverables: expect Opus-4.8-class fallback behavior; design for the structural refusal category. Frontier-LLM-dev work needs a Phase 5 delta flag (invisible throttling, no fallback signal). Binary RE on Fable is blocked — route to Mythos 5 or accept blockage. |
...
| **Mid-conversation system messages — CONFIRMED still available** (card 4884–4916, an Opus 4.8 carryforward feature exercised in the audit scaffold). | Carry forward the 4.8-era recommendation: revise instructions late in a long run with a `role:"system"` message after a user turn — preserves prompt-cache hits on earlier turns. |
...
**Fallback-aware harness** — on API, the default is **no auto-fallback**; blocked requests return a structured refusal category. Opt in to server-side fallback to a designated model (e.g. `claude-opus-4-8`); the fallback-model identifier is reflected in the response object. Read both the refusal category and the fallback-model field. (Card 739–744; Meta-Rule 16.)
...
### Parameters to remove from any 4.8-or-earlier draft

| Remove | Why |
|--------|-----|
| `temperature`, `top_p`, `top_k` (non-default) | 400 error on Fable 5 (unchanged from 4.8). |
...
- **Fable 5 and Mythos 5 are thinking-only** (card 3082–3088). There is no thinking-off configuration; the old "adaptive thinking OFF by default on raw API" claim is inverted. A thinking-dependent prompt may behave differently on a fallback turn that runs on Opus 4.8 (which still supports a thinking-off mode).
...
- **Two-model architecture.** Mythos 5 is the frontier weights; Fable 5 wraps the same weights with classifier safeguards (cyber, bio, chem, distillation, frontier-LLM-dev) and **silent fallback to Opus 4.8** on consumer surfaces when classifiers fire. For finance / M&A / corp-dev / general analytical work the classifiers are irrelevant and Fable 5 = Mythos 5 capability. For cyber/bio work, expect Opus 4.8-class fallback behavior. (Card 29–35, 668–672, 3091–3098.)
...
- **Mid-conversation system messages** remain available (Opus 4.8 carryforward, exercised in audit scaffold; card 4884–4916). Use after a user turn to revise instructions without restating the system prompt — preserves prompt-cache hits.
...
Prefilling returns 400; partial-turn prefill is not generally available externally for Opus 4.6+ / Mythos 5 / Fable 5 (card 4530–4535). Replacements: `output_config.format` with a JSON schema for forced JSON; system-prompt direction ("Respond directly without preamble…") for preamble suppression; move continuations into the user turn. `output_format={...}` (top-level) is deprecated — migrate to `output_config={"format": {...}}`.
...
- **Microsoft Foundry context split: UNKNOWN — Confidence: Medium pending Phase 1.5.** (The 4.8-era 200K split is not currently re-verified for Fable 5.)
...
- The 1M context is usable; optimized prompts may freely instruct multi-document analysis and large-codebase navigation. GraphWalks Parents-1M jumped from 83.3% (Opus 4.8) to 97.5% (Fable/Mythos 5; card 9789–9844) — chunking workarounds are largely obsolete at this scale.
...
- **Honesty changed shape, not direction.** Fable 5 attempts more and gets more right (4 of 4 factuality benchmarks ahead of Opus 4.8) — but also wrong more often in absolute terms (Opus 4.8 still has the lowest incorrect-rate on 3 of 4; card 5871–5888). Missing-context honesty regressed (unavailable-tool 95→87%; missing-reference 91→82%; 5970–5978). MASK honesty under pressure regressed (5926–5930). Flawed-data detection holds at Opus 4.8 parity but framing softened ("design conventions" vs mistakes; 6125–6170). Code-summary honesty showed a small regression (6182–6184). Fabricated SHA256 checksums and a fabricated user-approval message are documented at <0.01% (4765–4770). **The completion-verification guardrail (Meta-Rule 11) is now more important, not less** — defense-in-depth against larger-blast-radius destructive actions (5610–5666) and against missing-input fabrication. See [fable-5-system-card.md](fable-5-system-card.md) §3, §4, §6 for the full eval table and behavioral conclusions.
- **Refusal calibration INVERTED.** Over-refusal collapsed ~35× on API (0.35% → 0.01%, card 3164–3179) — the **lowest** safety-research refusal rate ever measured. Security work (pentesting, vuln research) is now constitutionally carved out (9048–9077). Strip "this is legitimate research" boilerplate — it is now noise and possibly counterproductive. **Exception:** one-sided persuasive/advocacy content is refused **more** (3681–3690); prompts asking for one-sided essays should include "do not include counterarguments" explicitly. See `task-heuristics.md` row 31.
- **Prompt injection IMPROVED on most surfaces, with two regressions.** ART k=100 4.8% vs Opus 4.8 9.6% (best ever; card 4098–4101). Shade coding 0.45% vs 7.03% (4154–4221). Shade computer-use 0.46% vs 5.11% (4240–4300). Browser-use under **legacy** safeguards regressed (6.5% vs 0.5%); under **updated** safeguards drops to 0% (4356–4426). Prefill toward misaligned behavior is far more effective (UK AISI compromise-continuation 14% vs 1%; **69% covert** reasoning-output discrepancy; 5513–5525). Keep untrusted-content discipline at full strength; re-anchor it to inherent risk, not a stale model delta. See `task-heuristics.md` rows 35, 48 and `fable-5-system-card.md` §7.

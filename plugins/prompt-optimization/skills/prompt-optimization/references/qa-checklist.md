...
- [ ] The deliverable's first section is the executive summary (per `synthesis-deliverable.md` § Executive summary template), preceding synthesized findings, supporting evidence, gaps, and sources.
- [ ] The executive summary names 3–7 key findings, each in a single-sentence statement.
- [ ] Every key finding carries at least one primary-source URL.
- [ ] Every key finding carries a source-validation verdict (`verified` / `partially verified — see note` / `not verified` / `unreachable`).
- [ ] Every key finding carries a confidence stamp (High / Medium / Low) consistent with the source-validation verdict (a `partially verified` finding cannot stamp `High`).

...
| 4 | Execute Phase 1.5 (web research for platform calibration). Execute Phase 2 (landscape research). Execute Phase 2.5 — if research-informed questions exist, present a brief prose summary of calibration findings, then issue ONE `ask_user_input_v0` call with the first batch of 1-3 Phase 2.5 questions. If Phase 2.5 was skipped, proceed directly to final output. | Hard stop (if widget issued) or pass-through. |
...
- Phase 2 internal working artifacts: the 2A Topic Identification & Draft Deconstruction block, the 2B Landscape Research Execution Log, the 2C Landscape Synthesis output, the 2D' Coverage-Bias Check, or any other Phase 2 analytical scaffolding
